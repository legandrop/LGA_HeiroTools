...
v11: agrega el frame number de los attachments de la version en SG
...
class ShotGridManager:
    ...
    def extract_frame_from_attachment_name(self, attachment_name):
        ...
        # Patron 1: Archivos subidos desde Flow Push: annot_version_49735.1015.jpg
        frame_match = re.search(
            r"annot_version_\d+\.(\d+)\.(jpg|jpeg|png)$", attachment_name
        )
        if frame_match:
            return frame_match.group(1)

        # Patron 2: Archivos subidos desde web de Flow: annot_version_51150.16.png
        frame_match = re.search(r"\.(\d+)\.(png|jpg|jpeg)$", attachment_name)
        if frame_match:
            return frame_match.group(1)

        # Patron 3: Otros formatos posibles con frame al final
        frame_match = re.search(r"_(\d+)\.(png|jpg|jpeg)$", attachment_name)
        if frame_match:
            return frame_match.group(1)
                                    ...
                                    if note.get("attachments"):
                                        for attachment_ref in note["attachments"]:
                                            ...
                                            if attachment_details:
                                                # Intentar extraer frame number de multiples fuentes
                                                frame_number = self.sg_manager.extract_frame_from_attachment_name(
                                                    attachment_ref["name"]
                                                )

                                                # Si no se pudo extraer del nombre, intentar con campos de ShotGrid
                                                if frame_number == "Unknown":
                                                    if attachment_details.get(
                                                        "sg_frame_number"
                                                    ):
                                                        frame_number = str(
                                                            attachment_details[
