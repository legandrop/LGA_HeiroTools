...
def check_user_has_project(user_name, project_name):
    ...
    if not all([sg_url, sg_login, sg_password]):
        print(f"ERROR: No se pudieron obtener las credenciales de Flow desde SecureConfig.")
        return False, None
    ...
    try:
        ...
        debug_print(f"[{time_project_search*1000:.2f}ms] Búsqueda del proyecto '{project_name}' completada")
        ...
        debug_print(f"Proyecto encontrado: {project_to_check['name']} (ID: {project_id_to_check})")
        ...
        debug_print(f"[{time_project_check*1000:.2f}ms] Verificación completa de proyecto")
...
def main():
    ...
    print(f"\n{'='*70}")
    if has_project:
        print(f"RESULTADO: ✓ SÍ - El usuario '{user_name}' TIENE asignado el proyecto '{project_name}'")
        if project_info:
        ...
        print(f"RESULTADO: ✗ NO - El usuario '{user_name}' NO tiene asignado el proyecto '{project_name}'")
        if project_info:
