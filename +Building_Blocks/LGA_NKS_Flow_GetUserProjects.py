...
def get_user_projects(user_name):
    ...
    if not all([sg_url, sg_login, sg_password]):
        print(
            "ERROR: No se pudieron obtener las credenciales de Flow desde SecureConfig."
        )
        return []
    ...
    try:
        ...
        for project_ref in projects:
            ...
            if project_id:
                ...
                project = sg.find_one(
                    "Project", [["id", "is", project_id]], ["id", "name", "code"]
                )
                if project:
                    project_info = {
                        "id": project["id"],
                    ...
                    debug_print(
                        f"  - Proyecto: {project_info['name']} (ID: {project_info['id']}, Code: {project_info['code']})"
                    )
...
def unassign_project_from_user(user_name, project_name):
    ...
    try:
        ...
        debug_print(
            f"[{time_project_search*1000:.2f}ms] Búsqueda de proyecto completada"
        )
        ...
        for proj_ref in current_projects:
            if proj_ref.get("id") == project_id:
                project_found = True
                debug_print(
                    f"Proyecto '{project_name}' encontrado en la lista del usuario"
                )
            else:
        ...
        if not project_found:
            return (
                False,
                f"El proyecto '{project_name}' no está asignado al usuario '{user_name}'.",
            )
        ...
        if result:
            ...
            return (
                True,
                f"Proyecto '{project_name}' desasignado exitosamente del usuario '{user_name}'.",
            )
        else:
...
def assign_project_to_user(user_name, project_name):
    ...
    try:
        ...
        if project_already_assigned:
            return (
                False,
                f"El proyecto '{project_name}' ya está asignado al usuario '{user_name}'.",
            )
        ...
        if result:
            ...
            return (
                True,
                f"Proyecto '{project_name}' asignado exitosamente al usuario '{user_name}'.",
            )
        else:
            return False, "ERROR: No se pudo actualizar el usuario en Flow."
...
def main():
    ...
    success_unassign, message_unassign = unassign_project_from_user(
    ...
    if success_unassign:
        ...
        print(f"✗ {message_unassign}")
    print(
        f"Tiempo total de desasignación: {time_unassign*1000:.2f}ms ({time_unassign:.3f}s)"
    )
    ...
    print(
        f"Total:         {(time_unassign + time_assign)*1000:.2f}ms ({(time_unassign + time_assign):.3f}s)"
    )
    print(f"{'='*70}\n")
