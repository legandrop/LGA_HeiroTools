"""
LGA_NKS_Flow_Pull_DoScan v1.0 | Versión que usa doScan para escanear versiones
Esta versión ejecuta doScan para asegurar que todas las versiones estén disponibles
"""
...
class VersionManager:
    """Clase para manejar cambios de versión usando doScan"""

    def __init__(self, sg_manager):
        self.sg_manager = sg_manager

    def get_highest_version(self, binItem):
        """Obtiene la version mas alta de un binItem usando doScan con timeout para evitar cuelgues."""
        debug_print(f"Obteniendo versiones con doScan para {binItem.name()}")

        # Crear VersionScanner
        ...
        if not versions:
            debug_print(f"No se encontraron versiones disponibles")
            return None
    ...
    def change_clip_to_highest_version(self, clip):
        """Cambia un clip a su versión más alta usando doScan."""
...
def run_pull_with_doscan():
    """Ejecuta el pull usando doScan para el cambio de versiones"""
