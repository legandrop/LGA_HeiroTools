...
def find_frame_only_effect(track, base_name="Frame_Only"):
    ...
    debug_print(
        f"✅ Encontrado efecto '{selected_name}' (número más bajo de {len(found_effects)} encontrados)"
    )
...
def print_box_values():
    ...
    if not target_clip:
        debug_print(
            f"⚠️ No se encontró ningún efecto que empiece con '{CLIP_NAME}' en el track '{TRACK_NAME}'."
        )
        ...
        try:
            ...
            debug_print(
                f"   Asegúrate de que LGA_NKS_FrameNumber_Create.py esté disponible"
            )
            return
        ...
        if qimage:
            ...
            try:
                ...
                if player:
                    ...
                    image_aspect = (
                        image_width / image_height if image_height > 0 else 1.0
                    )
                    viewer_aspect = (
                        viewer_width / viewer_height if viewer_height > 0 else 1.0
                    )

                    debug_print(f"   Aspect ratios:")
                    debug_print(
                        f"      Imagen: {image_aspect:.4f} ({image_width}:{image_height})"
                    )
                    debug_print(
                        f"      Viewer: {viewer_aspect:.4f} ({viewer_width}:{viewer_height})"
                    )
                    ...
                    visible_width_raw = (
                        viewer_width / zoom if zoom > 0 else viewer_width
                    )
                    visible_height_raw = (
                        viewer_height / zoom if zoom > 0 else viewer_height
                    )
                    ...
                    if USE_ABSOLUTE_POSITION:
                        visible_width_in_image = min(
                            visible_width_in_image, image_width
                        )
                        visible_height_in_image = min(
                            visible_height_in_image, image_height
                        )
                    ...
                    try:
                        ...
                        debug_print(f"\n📍 Pan del viewer:")
                        debug_print(
                            f"   X viewer: {pan_x_viewer:.2f} -> imagen: {pan_x_image:.2f}"
                        )
                        debug_print(
                            f"   Y viewer: {pan_y_viewer:.2f} -> imagen: {pan_y_image:.2f}"
                        )
                        ...
                        visible_x_offset = visible_center_x - (
                            visible_width_in_image / 2
                        )
                        visible_y_offset = visible_center_y - (
                            visible_height_in_image / 2
                        )

                        debug_print(f"\n📐 Área visible:")
                        debug_print(
                            f"   Centro imagen: ({image_center_x:.2f}, {image_center_y:.2f})"
                        )
                        debug_print(
                            f"   Centro visible: ({visible_center_x:.2f}, {visible_center_y:.2f})"
                        )
                        debug_print(
                            f"   Offset: X={visible_x_offset:.2f}, Y={visible_y_offset:.2f}"
                        )
                        debug_print(
                            f"   Dimensiones visibles: W={visible_width_in_image:.2f}, H={visible_height_in_image:.2f}"
                        )
                    except Exception as e:
                        debug_print(
                            f"   ⚠️ Error obteniendo pan de player.translation(): {e}"
                        )
                        ...
                        if visible_width_in_image >= image_width:
                            ...
                            visible_x_offset = (
                                image_width - visible_width_in_image
                            ) / 2
                        ...
                        if visible_height_in_image >= image_height:
                            ...
                            visible_y_offset = (
                                image_height - visible_height_in_image
                            ) / 2

                        debug_print(
                            f"   ⚠️ Fallback centrado: X={visible_x_offset:.2f}, Y={visible_y_offset:.2f}"
                        )
        ...
        if isinstance(box_value, (tuple, list)) and len(box_value) >= 4:
            ...
            if USE_ABSOLUTE_POSITION:
                ...
                if visible_y_offset <= 0:
                    ...
                    debug_print(
                        f"   Usando new_y = BOTTOM_MARGIN = {BOTTOM_MARGIN} (bottom de la imagen)"
                    )
                else:
                    # El área inferior está cropeada, usar el bottom del área visible
                    new_y = (
                        visible_y_offset + BOTTOM_MARGIN
                    )  # Bottom del box a 30px del bottom del área visible
                    debug_print(f"\n⚠️ Área inferior cropeada:")
                    debug_print(f"   visible_y_offset={visible_y_offset:.2f} > 0")
                    debug_print(
                        f"   Usando new_y = visible_y_offset + BOTTOM_MARGIN = {new_y:.2f}"
                    )
            ...
            if not USE_ABSOLUTE_POSITION:
                ...
                debug_print(
                    f"   Área visible - Bottom: X={visible_x_offset:.2f}, Y={visible_y_offset:.2f}"
                )
                debug_print(
                    f"   Área visible - Top: X={visible_x_offset + visible_width:.2f}, Y={visible_y_offset + visible_height:.2f}"
                )
                debug_print(
                    f"   Posición esperada: X={expected_x:.2f}, Y={expected_y:.2f}"
                )
                debug_print(f"   Posición calculada: X={new_x:.2f}, Y={new_y:.2f}")
                debug_print(
                    f"   Diferencia: X={new_x - expected_x:.2f}px, Y={new_y - expected_y:.2f}px"
                )
                debug_print(
                    f"   Desde área visible: X={new_x - visible_x_offset:.2f}px (esperado: {LEFT_MARGIN}px)"
                )
                debug_print(
                    f"   Desde área visible: Y={new_y - visible_y_offset:.2f}px (esperado: {BOTTOM_MARGIN}px)"
                )
                debug_print(
                    f"   ⚠️ Si Y está muy alto visualmente, new_y debería ser MAYOR que expected_y"
                )
                debug_print(
                    f"   ⚠️ En Hiero Y=0 es bottom, entonces Y más grande = más arriba"
                )
                    ...
                    try:
                        ...
                        if viewer:
                            ...
                            if player:
                                ...
                                pan_y_image_sin_inversion_test = (
                                    pan_y_viewer_test / zoom_test
                                    if zoom_test > 0
                                    else pan_y_viewer_test
                                )
                                visible_center_y_sin_inversion_test = (
                                    image_height / 2 - pan_y_image_sin_inversion_test
                                )
                                visible_y_offset_sin_inversion_test = (
                                    visible_center_y_sin_inversion_test
                                    - (visible_height / 2)
                                )
                                expected_y_sin_inversion = (
                                    visible_y_offset_sin_inversion_test + BOTTOM_MARGIN
                                )

                                debug_print(f"\n💡 Análisis del problema:")
                                debug_print(
                                    f"   Con inversión: visible_y_offset={visible_y_offset:.2f} -> expected_y={expected_y:.2f} -> new_y={new_y:.2f}"
                                )
                                debug_print(
                                    f"   Sin inversión: visible_y_offset={visible_y_offset_sin_inversion_test:.2f} -> expected_y={expected_y_sin_inversion:.2f}"
                                )
                                debug_print(
                                    f"   Diferencia en expected_y: {abs(expected_y - expected_y_sin_inversion):.2f}px"
                                )
                                ...
                                debug_print(
                                    f"   new_y actual (con inversión): {new_y:.2f}"
                                )
                                debug_print(
                                    f"   new_y esperado (sin inversión): {expected_y_sin_inversion:.2f}"
                                )
                                debug_print(
                                    f"   ⚠️ DIFERENCIA: {diferencia_y:.2f}px MÁS ALTO de lo que debería estar"
                                )
                                debug_print(
                                    f"   ⚠️ El box está {diferencia_y:.2f}px más arriba de donde debería estar visualmente"
                                )
                                debug_print(
                                    f"   ⚠️ Si el box está muy alto, debería estar en {expected_y_sin_inversion:.2f} en lugar de {new_y:.2f}"
                                )
                                debug_print(
                                    f"   ⚠️ CONCLUSIÓN: El problema está en la INVERSIÓN del pan Y"
                                )
                                debug_print(
                                    f"   ⚠️ SOLUCIÓN: NO deberíamos invertir el pan Y (quitar el signo negativo)"
                                )
                    except:
                        pass
            ...
            debug_print(
                f"Valor obtenido: {box_value} (Tipo: {type(box_value).__name__})"
            )
    except Exception as e:
