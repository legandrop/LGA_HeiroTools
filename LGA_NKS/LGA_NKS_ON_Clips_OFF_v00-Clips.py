                        ...
                        if file_path:
                            ...
                            if version_match:
                                ...
                                if version_number == '00' or version_number == '000':
                                    ...
                                    debug_print(f"Clip '{item.name()}' desactivado (versión v{version_number}).")
                                else:
                                    item.setEnabled(True)
                                    debug_print(f"Clip '{item.name()}' activado (versión v{version_number}).")
                            else:
                                # No tiene patrón de versión _comp_vXX, activar
                                item.setEnabled(True)
                                debug_print(f"Clip '{item.name()}' activado (sin versión detectada).")
                        else:
                            item.setEnabled(True)
                            debug_print(f"Clip '{item.name()}' activado (sin ruta de archivo).")
            else:
