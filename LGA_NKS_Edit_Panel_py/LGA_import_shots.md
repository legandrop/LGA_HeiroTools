...
| `LGA_NKS_Edit_Panel_py/LGA_import_shots_transcode_queue_ui.py` | `TranscodeQueueWindow`, `show_queue_window()`, UI no modal de `Open Queue`, historial visual, `Keep on top` persistente |
...
| `LGA_NKS_Edit_Panel_py/LGA_import_shots_transcode_queue_ui.md` | Especificacion visual y tecnica de la ventana `Open Queue`: tabla global, columnas Shot/Plate/Duracion/Estado, historial visual, `Clear Completed`, `Keep on top` |
