...
def debug_print(*message):
    ...
    if DEBUG:
        ...
        print(f"[DEBUG] {msg}")  # Imprimir inmediatamente
        # También almacenar en lista para el caso del ScanWorker
...
class ProjectItem(QtWidgets.QWidget):
    ...
    def __init__(self, project_info, projects_panel, parent=None):
        super(ProjectItem, self).__init__(parent)
        self.project_info = project_info
        self.projects_panel = projects_panel  # Referencia al ProjectsPanel
        self.is_open = False
        self.sequences = []
        self.setup_ui()
    ...
    def show_sequences(self):
        ...
        for seq_name in sorted(self.sequences):
            ...
            # NO setear is_project_label para que sea False (sequence label)
            # Instalar event filter para hover
            seq_label.installEventFilter(self.projects_panel)
...
class ProjectsPanel(QtWidgets.QWidget):
    ...
    def setup_ui(self):
        ...
        if os.path.exists(refresh_icon_path) and os.path.exists(refresh_hover_icon_path):
            ...
            debug_print("🔧 Instalando eventFilter en refresh_button")
    ...
    def eventFilter(self, obj, event):
        ...
        # Debug deshabilitado para evitar sobrecarga de logs
        ...
        if obj == self.refresh_button:
            if event.type() == QtCore.QEvent.Enter:
                ...
                if hasattr(self, 'refresh_icon_normal'):
                    self.refresh_button.setIcon(self.refresh_icon_normal)
                return True  # Evento manejado
            # Para otros eventos del botón refresh, no manejar
            return False

        # Manejar hover de los project labels y sequence labels
        elif isinstance(obj, QtWidgets.QLabel) and obj != self.refresh_button:
            # Es un QLabel (project o sequence)
    ...
    def update_projects_display(self):
        ...
        for proyecto_info in sorted(self.proyectos_encontrados, key=lambda x: x.get("nombre_base", "")):
            ...
            debug_print(f"🔧 Instalando eventFilter en project_label: {nombre_base}")
