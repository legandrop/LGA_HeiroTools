...
def get_knob_info(node, knob_name):
    ...
    try:
        ...
        knob_info = {
            ...
            'class_name': knob.__class__.__name__ if hasattr(knob, '__class__') else 'Unknown'
        }
...
def explore_frame_only_effect():
    ...
    if not target_effect:
        debug_print(f"❌ No se encontró el soft effect '{CLIP_NAME}' en el track '{TRACK_NAME}'.")
    ...
    effect_properties['timeline_out'] = safe_get_value(target_effect, 'timelineOut', None)
    debug_print(f"  Timeline Out: {effect_properties['timeline_out']}")
    
    if effect_properties['timeline_in'] is not None and effect_properties['timeline_out'] is not None:
        effect_properties['duration'] = effect_properties['timeline_out'] - effect_properties['timeline_in']
    ...
    try:
        effect_properties['track_name'] = target_effect.parentTrack().name() if target_effect.parentTrack() else None
    ...
    for item in items:
        ...
        if isinstance(effect_item, hiero.core.EffectTrackItem):
            if effect_item.name() != CLIP_NAME:
                ...
                if effect_info['timeline_in'] is not None and effect_info['timeline_out'] is not None:
                    effect_info['duration'] = effect_info['timeline_out'] - effect_info['timeline_in']
                other_effects.append(effect_info)
                debug_print(f"  - {effect_info['name']}: In={effect_info['timeline_in']}, Out={effect_info['timeline_out']}, Duration={effect_info.get('duration', 'N/A')}")
